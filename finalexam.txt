...
void user_interface()
{
    ...
    while (true)
    {
        ...
        else if (sscanf(buffer, "add %d %d", &number, &count) == 2)
        {
            ...
            if (success)
                printf("%d instances of %d are added successfully.\n", count, number);
            else
                printf("Failed to add %d instances of %d.\n", count, number);
        }
        else if (sscanf(buffer, "delete %d", &number) == 1)
        {
        ...
        else if (sscanf(buffer, "delete %d all", &number) == 1)
        {
            bool success = true;
            while (find(number) > 0)
            {
...
void stats()
{
    ...
    printf("a. Average: %.2f, Standard deviation: %.2f, Median: %.2f\n", mean, std, median);

    printf("b. Min: %d, Max: %d, Percentage of allowed values: %.2f%%\n", min, max, non_neg_count * 100.0 / nt);

    printf("c. Length of longest sequence of duplicates: %d, Length of longest sequence of consecutive numbers: %d, Length of longest sequence of -1s: %d\n", longest_dup, longest_cons, longest_neg);

    printf("d. Percentage of nums array devoted to storing -1s: %.2f%%, Average interval between -1s: %.2f, Max interval between -1s: %d\n", neg_count * 100.0 / nt, nt * 1.0 / (neg_count + 1), longest_cons + longest_dup + 1);
    ...
    printf("e. The numbers are stored non-descending in nums: %s\n", valid ? "true" : "false");
}
int search(int target)
{
    int low = 0, high = nt - 1;
